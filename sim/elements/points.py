...
class ArrowsElement(ElementBase):
    ...
    def __init__(self, data: np.ndarray, parameters: Dict[str, Any] = None):
        ...
        Args:
            data (:class:`~numpy.ndarray`):
                The positions and directions of all points
            parameters (dict):
                Additional parameters. Call
                :meth:`~PointsElement.show_parameters` for details.
        """
        ...
        self._data = np.atleast_2d(data)
        if self.data.ndim != 2:
            raise ValueError("`data` must be a sequence of positions and directions")
        if self.data.shape[1] % 2 == 1:
            raise ValueError("The data must contain positions and directions")
        self.dim = self.data.shape[1] // 2
    ...
    def from_position_direction(
        ...
        data = np.c_[positions, directions]
    ...
    def positions(self) -> np.ndarray:
        return self.data[:, : self.dim]  # type: ignore

    @positions.setter
    def positions(self, value: np.ndarray) -> None:
        self.data[:, : self.dim] = value
